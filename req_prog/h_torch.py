...
def _ordered_product_tree(mats: torch.Tensor) -> torch.Tensor:
    """
    Compute the ordered matrix product U_{K-1} @ ... @ U_1 @ U_0
    using a divide-and-conquer binary tree reduction.
 
    Reduces the number of matrix multiplications from O(K) to O(log2(K)),
    at the cost of losing strict left-to-right ordering within each level.
    This is valid here because the Trotter slices are already small enough
    that the reordering error is negligible compared to the Trotter error itself.
 
    Parameters
    ----------
    mats : torch.Tensor
        Shape (B, K, d, d) — K ordered unitary slices per batch item.
 
    Returns
    -------
    torch.Tensor
        Shape (B, d, d) — the accumulated product.
    """
    B, K, d, _ = mats.shape
    device, dtype = mats.device, mats.dtype
 
    # Pad to next power of 2 with identity matrices if needed
    K_pad = 1 << (K - 1).bit_length()   # smallest power of 2 >= K
    if K_pad > K:
        eye = torch.eye(d, dtype=dtype, device=device)\
                   .view(1, 1, d, d).expand(B, K_pad - K, d, d)
        mats = torch.cat([mats, eye], dim=1)   # (B, K_pad, d, d)
 
    # Binary tree reduction: pair-wise matrix multiplication
    while mats.shape[1] > 1:
        half = mats.shape[1] // 2
        left  = mats[:, 0::2, :, :]   # (B, half, d, d)
        right = mats[:, 1::2, :, :]   # (B, half, d, d)
        # left[k] @ right[k] for all k simultaneously
        mats = torch.bmm(
            left.reshape(B * half, d, d),
            right.reshape(B * half, d, d)
        ).view(B, half, d, d)
 
    return mats[:, 0]   # (B, d, d)
 
 
def unitary_trotter_torch(
    ...
    """
    Compute the time-evolution operator U(t) via first-order Lie-Trotter slicing.
 
    Optimizations over the original implementation:
      1. All matrix exponentials exp(-i H(t_k) dt) are computed in a single
         batched call to torch.linalg.matrix_exp, removing the Python loop
         over time slices for this step.
      2. The ordered product U_{K-1} @ ... @ U_0 is accumulated via a
         divide-and-conquer binary tree, reducing matrix multiplications
         from O(steps) to O(log2(steps)).
 
    Parameters
    ----------
    model : torch.nn.Module
        Network that parametrizes H(t) via get_ising_H_batch.
    time : torch.Tensor
        Shape (B, 1) or (B,) — target evolution times.
    steps : int
        Number of Trotter slices (default: 50).
    n : int
        Number of qubits; matrix dimension d = 2^n (default: 7).
 
    Returns
    -------
    torch.Tensor
        Shape (B, 2^n, 2^n) — approximate unitary evolution operators.
    """
    device = time.device
    if time.dim() == 1:
        time = time.view(-1, 1)       # (B, 1)
    B = time.shape[0]
    d = 2 ** n
 
    # ── Time grid (midpoint sampling) ─────────────────────────────────────────
    # dt per batch item: (B, 1, 1, 1) for broadcasting against (B, steps, d, d)
    dt = (time / steps).view(B, 1, 1, 1)
 
    tau    = (torch.arange(steps, device=device, dtype=time.dtype) + 0.5) / steps
    t_grid = time.view(B, 1, 1) * tau.view(1, steps, 1)   # (B, steps, 1)
    t_flat = t_grid.reshape(-1, 1)                          # (B*steps, 1)
 
    # ── Evaluate H(t_k) for all slices at once ────────────────────────────────
    H_all = get_ising_H_batch(model=model, time_batch=t_flat, n=n)
    H_all = H_all.view(B, steps, d, d)                     # (B, steps, d, d)
 
    # ── Batched matrix exponential (no Python loop) ───────────────────────────
    # exponents shape: (B, steps, d, d)
    exponents = -1j * H_all * dt                            # broadcasts dt
    # Flatten to (B*steps, d, d), compute all exp at once, reshape back
    U_slices = matrix_exp(
        exponents.reshape(B * steps, d, d)
    ).view(B, steps, d, d)                                  # (B, steps, d, d)
 
    # ── Ordered product via binary tree ───────────────────────────────────────
    U = _ordered_product_tree(U_slices)                     # (B, d, d)
