...
def train_model_h(
    ...
    if delta_t in [0.1, 0.25]:
        data_train  = np.load(f"dataframe/data_model{N_qbits}_deltat-{delta_t}.npz")["arr_0"]
    else:
        data_train  = np.load(f"dataframe/data_model{N_qbits}_deltat-{delta_t}.npz")["arr"]
    ...
    for epoch in tqdm(range(num_epochs), desc=f'Training model{N_qbits} with delta_t={delta_t}', unit='epoch'):
        ...
        for dl in selected_dataloaders:
            for (rho_in, t_scalar), rho_out in dl:
                ...
                t_scalar = t_scalar.requires_grad_(True)
        
                if method == "magnus":
                    U_pred = unitary_magnus_torch(model = model, time = t_scalar, n = N_qbits)
                    U_t    = unitary_magnus_torch(model = model, time = t_u, n = N_qbits)
                elif method == "trotter":
                    U_pred = unitary_trotter_torch(model = model, time = t_scalar, n = N_qbits)
                    U_t    = unitary_trotter_torch(model = model, time = t_u, n = N_qbits)
                else:
                    print("Error! Please insert method as magnus or trotter")
                    break
        ...
        if epoch % 10 == 0:
            # Save the model
            torch.save(model.state_dict(), os.path.join("models", f"model{N_qbits}_deltat-{delta_t}-{method}.pth"))
            ...
            df.to_csv(os.path.join("./data_loss", f"dataloss-model{N_qbits}-delta_t{delta_t}-{method}.csv"), index=False)
